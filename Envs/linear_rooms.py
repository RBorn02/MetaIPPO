...
from simple_playgrounds.elements.collection.activable import ActivableByGem, RewardOnActivation
...
class LinLandmarksEnv(MultiAgentEnv):
    def __init__(self, config):
        super(LinLandmarksEnv, self).__init__()
    ...
    def spawn_landmarks(self):
        ...
        random.shuffle(possible_pos)
        self.fountains = []
        for agent, i in zip(self._active_agents, range(self.num_agents)):
            coordinates = CoordinateSampler(possible_pos[i], area_shape="rectangle", size=(20, 40))
            reward_fountain = CustomRewardOnActivation(agent.name, physical_shape="circle", radius=7, texture=ColorTexture(color=possible_colors[i], size=7),
                            name="reward_{0}".format(i), temporary=True)
            self.playground.add_element(reward_fountain, coordinates, allow_overlapping=False)
            self.fountains.append(reward_fountain)
    ...
    def compute_reward(self):
        ...
        for fountain in self.fountains:
            if fountain.activated:
                activated_fountains.append(True)
            else:
                activated_fountains.append(False)

        
        for agent in self._active_agents:
            if all(activated_fountains):
                reward = 1.0
            else:
                reward = 0.0
            rewards[agent.name] = reward
...
class LinLandmarksEnvComm(MultiAgentEnv):
    def __init__(self, config):
        super(LinLandmarksEnvComm, self).__init__()
...
if __name__ == "__main__":
    ...
    config = {"num_landmarks": 1,
              ...
              "single_reward": False,
              "random_assign": True}
    env = LinLandmarksEnvComm(config)
