...
from Utils.train_utils import build_config, build_storage_pop, handle_dones, print_info, build_storage_from_batch_pop, record_video
...
parser.add_argument("--total_steps", type=int, default=2.5*10e6,
        ...
        while True:
            try:
                if all(np.array(done, dtype=bool)):
                    ...
                    #Restart the workers and assing new agents
                    assign_list[:] = []
                    agent_ids = [i for i in range(config["env_config"]["num_agents"])]
                    random.shuffle(agent_ids)
                    for w, a in zip(range(0, config["num_workers"], workers_per_agent), range(0, config["env_config"]["num_agents"], 2)):
