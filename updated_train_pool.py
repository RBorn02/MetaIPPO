...
from policy import LSTM_PPO_Policy_Pop
from agent import LSTMAgent, CommsLSTMAgent
from Envs.environment_handler import EnvironmentHandlerPop, EnvironmentHandler
...
args.batch_size = int(args.rollout_steps // args.num_agents * 2)
...
config["env_config"]["agents_per_env"] = 2
...
#Build the policies
policy_dict = {"agent_{0}".format(a): LSTM_PPO_Policy_Pop(config, agent_dict["agent_{0}".format(a)], optimizer_dict["agent_{0}".format(a)]["optimizer"]) 
               for a in range(config["env_config"]["num_agents"])}



def rollout(pid, policy_dict, train_queue, done, config, assign_list):
    try:
        ...
        if config["env_config"]["env_name"] in ["MultiAgentLandmarksComm", "LinRoomEnvComm", "LinLandmarksEnvComm"]:
            next_obs, next_messages_in, next_contact, _ = env.reset_all([i for i in range(config["env_config"]["num_envs"])], assigned_agents)
        else:
            next_obs, next_contact, _ = env.reset_all([i for i in range(config["env_config"]["num_envs"])], assigned_agents)

        next_dones = {"agent_{0}".format(a): torch.zeros((1, config["env_config"]["num_envs"])).to(device) for a in assigned_agents}
        last_actions = {"agent_{0}".format(a): storage["agent_{0}".format(a)]["actions"][0].to(device) for a in assigned_agents}
        last_rewards = {"agent_{0}".format(a): storage["agent_{0}".format(a)]["rewards"][0].to(device) for a in assigned_agents}
        ...
        try:
            if bool(done[pid]) is False:
                ...
                if config["env_config"]["env_name"] in ["MultiAgentLandmarksComm", "LinRoomEnvComm", "LinLandmarksEnvComm", "TreasureHuntComm"]:
                    ...
                    next_obs, next_messages_in, rewards, dones, next_contact, infos = env.step(input_dict, assigned_agents)
                
                else:
                    actions = torch.cat([storage["agent_{0}".format(a)]["actions"][rollout_step].unsqueeze(dim=1)
                                    for a in assigned_agents], dim=1)
    
                    next_obs, rewards, dones, next_contact, infos = env.step(actions.cpu(), assigned_agents)
                    ...
                    if dones["__all__"][e]:
                        for a in assigned_agents:
                            if config["env_config"]["env_name"] in ["MultiAgentLandmarksComm", "LinRoomEnvComm", "LinLandmarksEnvComm", "TreasureHuntComm"]:
                                ...
                                reset_obs, reset_contact, _ = env.reset(e, assigned_agents)
                                next_obs["agent_{0}".format(a)][0][e] = reset_obs["agent_{0}".format(a)].to(device)
                                next_contact["agent_{0}".format(a)][0][e] = reset_contact["agent_{0}".format(a)].to(device)
...
if __name__ == "__main__":
    ...
    with mp.Manager() as manager:
        ...
        #assign agents
        #assert(config["num_workers"] % config["env_config"]["num_agents"] == 0)
        #assert(config["num_workers"] >= config["env_config"]["num_agents"])

        assign_list = manager.list()
        
        # workers_per_agent = config["num_workers"] // config["env_config"]["num_agents"] * 2 # 2 because of two agents per env
        # agent_ids = [i for i in range(config["env_config"]["num_agents"])]
        # random.shuffle(agent_ids)
        # for w, a in zip(range(0, config["num_workers"], workers_per_agent), range(0, config["env_config"]["num_agents"], 2)):
        #     ids = agent_ids[a:a+2]
        #     for i in range(workers_per_agent):
        #         assign_list.append(ids)

        agent_ids = [i for i in range(config["env_config"]["num_agents"])]
        combinations = list(itertools.combinations(agent_ids, 2))

        assert  config["num_workers"] % len(combinations) == 0

        workers_per_combination = config["num_workers"] // len(combinations)
        for idx in range(0, len(combinations)):
            ids = combinations[idx]
            for w in range(workers_per_combination):
                assign_list.append(ids)
        print(assign_list)
        ...
        for e in range(config["num_workers"]):
            p = mp.Process(target=rollout, args=([e, policy_dict, train_queue, done, config, assign_list]))
            p.start()
        ...
        while True:
            try:
                if all(np.array(done, dtype=bool)):
                    ...
                    if config["env_config"]["env_name"] in ["MultiAgentLandmarksComm", "LinRoomEnvComm", "LinLandmarksEnvComm", "TreasureHuntComm"]:
                        storage, next_obs, next_messages_in, next_dones, success_rate, goal_line, goal_line_success, next_contact, stage_success_info = build_storage_from_batch_pop(batch, config)
                    else:
                        storage, next_obs, next_dones, success_rate, goal_line, goal_line_success, next_contact, stage_success_info = build_storage_from_batch_pop(batch, config)
                    ...
                    if not config["debug"]:
                        ...
                        if update % update_ratio == 0:
                            ...
                            if (update * update_ratio) % config["record_video_every"] == 0:
                                ...
                                video_config["env_config"]["num_agents"] = 2
                                ...
                                video_policy_dict = {}
                                for i, a in zip(range(len(agent_ids)), agent_ids):
                                    video_policy_dict["agent_{0}".format(i)] = policy_dict["agent_{0}".format(a)]

                                if not os.path.exists(video_path):
                                    os.makedirs(video_path)
                                record_video(video_config, video_env, video_policy_dict, 4, video_path, update)
